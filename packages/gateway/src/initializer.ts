...
import { logger } from "@hive-sdk/utils/logger";
import { getDb, initializeDatabase, getDbPathLazy } from "@hive-sdk/storage/sqlite";
import { buildAgentLoop } from "@hive-sdk/agent/agent-loop";
import { AgentRunner } from "@hive-sdk/agent/providers/index";
import { ChannelManager } from "@hive-sdk/channels/manager";
import { syncToolsToFTS, syncSkillsToFTS, syncPlaybookToFTS } from "@hive-sdk/agent/context-compiler";
import { syncMCPToolsToFTS } from "@hive-sdk/mcp/tool-sync";
import { AgentService, createAgentService } from "@hive-sdk/agent/service";
import { mkdirSync, existsSync } from "node:fs";
import * as path from "node:path";
import { resolveAgentId, runStartupMigrations } from "@hive-sdk/storage/onboarding";
import { createMCPManager, type MCPClientManager } from "@hive-sdk/mcp";
import { setMCPManager } from "@hive-sdk/mcp/singleton";
import { startMCPHotReload } from "@hive-sdk/mcp/hot-reload";
import { initializeBrowserService } from "@hive-sdk/tools/web/browser-service";
import { activateBrowserTools } from "@hive-sdk/storage/onboarding";
...
export async function loadAgentConfigFromDB(
  ...
  try {
    ...
    if (providers.length > 0) {
      ...
      const { decryptApiKey } = await import("@hive-sdk/storage/crypto");
    ...
    for (const server of dbServers) {
      try {
        ...
        if (server.headers_encrypted && server.headers_iv) {
          const { decryptConfig } = await import("@hive-sdk/storage/crypto");
