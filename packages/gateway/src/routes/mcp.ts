...
import { encryptConfig, decryptConfig } from "@hive-sdk/storage/crypto.ts"
...
export async function handleGetMcpServerDetail(
  ...
  if (server.headers_encrypted && server.headers_iv) {
    try {
      headers = decryptConfig(server.headers_encrypted as string, server.headers_iv as string) as Record<string, string>
    } catch (e) {
