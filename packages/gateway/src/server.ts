import type { Config } from "@hive-sdk/config/loader";
import { loadConfig, getHiveDir } from "@hive-sdk/config/loader";
import { logger, onLogEntry } from "@hive-sdk/utils/logger";
...
import { ChannelManager } from "@hive-sdk/channels/manager";
import { AgentService } from "@hive-sdk/agent/service";
import { AgentRunner } from "@hive-sdk/agent/providers/index";
import type { IncomingMessage } from "@hive-sdk/channels/base";
...
import { getDb, getDbPathLazy, initializeDatabase } from "@hive-sdk/storage/sqlite";
import { seedAllData } from "@hive-sdk/storage/seed";
import { canvasManager } from "@hive-sdk/canvas/canvas-manager.ts";
import { subscribeCanvas, unsubscribeCanvas, emitCanvas, getCanvasSnapshot, removeCanvasComponent } from "@hive-sdk/canvas/emitter";
import { subscribeBridge, unsubscribeBridge } from "@hive-sdk/tools/bridge-events";
import { resolveCanvasInteraction } from "@hive-sdk/tools/canvas/index";
import { randomUUID } from "crypto";
import { decryptConfig } from "@hive-sdk/storage/crypto.ts";
import { resolveContext } from "./resolver";
import { voiceService } from "@hive-sdk/voice/index";
import { multimodalService } from "@hive-sdk/multimodal/index";
...
import { resolveUserId } from "@hive-sdk/storage/onboarding";
...
import { CronScheduler } from "@hive-sdk/scheduler/CronScheduler";
import { DAGScheduler, ParallelStrategy } from "@hive-sdk/scheduler/dag/index";
import { createTaskHandler, setSchedulerForCleanup } from "@hive-sdk/scheduler/integration";

import { setSchedulerInstance as setScheduleToolsInstance } from "@hive-sdk/tools/cron/index.ts";
...
export async function startGateway(config: Config): Promise<void> {
  ...
  if (!gatewaySetupMode) channelManager.onMessage(async (message: IncomingMessage) => {
    ...
    let contentParts: import("@hive-sdk/multimodal/types").ContentPart[] | undefined;
    ...
    if (message.image || message.document) {
      ...
      if (message.document) {
        try {
          ...
          const docImage: import("@hive-sdk/multimodal/types").ImageInput = {
            type: docInput.type,
            data: docInput.data,
    ...
    const messages = contentParts
      ? [{ role: "user" as const, content: [{ type: "text" as const, text: messageContentWithTime }, ...contentParts] as import("@hive-sdk/multimodal/types").ContentPart[] }]
  ...
  process.on("SIGTERM", async () => {
    ...
    try {
      const mod = await import("@hive-sdk/tools/web/browser-service");
    ...
    try {
      const { stopMCPHotReload } = await import("@hive-sdk/mcp/hot-reload");
      stopMCPHotReload();
