...
import { agentBus } from "@hive-sdk/events/agent-bus.ts";
import { emitCanvas } from "@hive-sdk/canvas/emitter.ts";
...
export const taskDelegateTool: Tool = {
  ...
  execute: async (params: Record<string, unknown>, config?: any) => {
    ...
    try {
      ...
      const { runAgentIsolated } = await import("@hive-sdk/agent/agent-loop.ts");
