...
describe("storage/bootstrap.ts", () => {
  test("ensureHiveDb is idempotent and marks bootstrapped", async () => {
    // Sin aserción sobre el estado inicial: `bootstrapped` es un flag de módulo
    // que `closeHiveDb()` no resetea, así que su valor de entrada depende de si
    // otro archivo de test ya llamó a ensureHiveDb en este mismo proceso.
    await ensureHiveDb();
