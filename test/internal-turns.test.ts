...
// El bloque "public history endpoint" de hive quedó fuera: cubre
// `handleGetChatHistory` de gateway/routes/chat.ts, y el gateway del SDK no
// incluye esas rutas. Lo que sí viaja es el filtro que consume la ruta —
// `isInternalSource` / `getRecentMessages` — que se verifica más abajo.
...
describe("internal turns: legacy role:system compat", () => {
  ...
  test("una fila legacy queda marcada como interna, así el historial público puede filtrarla", async () => {
    await addMessage("thread-i", "user", "hola");
    await writeLegacySystemRow("thread-i", "[Sistema] aviso viejo");

    const rows = await getRecentMessages("thread-i", 10);
    const visible = rows.filter((r) => !isInternalSource(r.source));

    expect(visible).toHaveLength(1);
    expect(visible[0].content).toBe("hola");
  });
});
